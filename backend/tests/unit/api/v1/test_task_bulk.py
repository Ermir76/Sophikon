...
@pytest.mark.asyncio
async def test_bulk_create_inclusive_finish_dates_when_auto_calculate_disabled(
    client: AsyncClient,
):
    await client.post(
        "/api/v1/auth/register",
        json={
            "email": "bulk-inclusive-finish@x.com",
            "password": "StrongPassword123!",
            "full_name": "Bulk Inclusive Finish",
        },
    )
    org_id = (
        await client.post(
            "/api/v1/organizations",
            json={"name": "Org Bulk Inclusive", "slug": "org-bulk-inclusive"},
        )
    ).json()["id"]
    proj_id = (
        await client.post(
    ...
    payload = {
        "tasks": [
            {"name": "Bulk One Day", "start_date": "2024-01-01", "duration": 480},
            {"name": "Bulk Two Day", "start_date": "2024-01-01", "duration": 960},
        ]
    }

    response = await client.post(f"/api/v1/projects/{proj_id}/tasks/bulk", json=payload)
    assert response.status_code == 200
    data = response.json()
    assert len(data["errors"]) == 0

    by_name = {task["name"]: task for task in data["tasks"]}
    assert by_name["Bulk One Day"]["finish_date"] == "2024-01-01"
    assert by_name["Bulk Two Day"]["finish_date"] == "2024-01-02"
