...
| FR-AU-010 | Implemented   | A1, A8   | Resend verification flow exists in auth backend and verify-email UI, now including a public recovery path for expired/blocked unverified users, visible resend feedback on the verify-email page, and backend-enforced 24-hour post-registration verification gating across login/refresh/protected access. |
...
| FR-KB-008 | Implemented   | A4, A10  | Kanban uses decoupled `selectedTaskId` (highlight) and `detailTaskId` (panel open) state; single-click selects, double-click opens floating `TaskDetailPanel`, explicit "View Details" openers still work, and the board remains interactive while the panel is open. |
